...
import static weka.core.converters.AbstractFileLoader.runFileLoader;
import weka.core.converters.BatchConverter;
import weka.core.converters.ConverterUtils;
import static weka.core.converters.Loader.BATCH;
import static weka.core.converters.Loader.NONE;

/**
 *
 * @author luciano
 */
public class CSVLoader2 extends AbstractFileLoader implements BatchConverter,
  OptionHandler {

    final static Logger logger = Logger.getLogger(CSVLoader2.class);
  ...
  private FastVector getInstance(StreamTokenizer tokenizer) throws IOException {
    ...
    logger.info(tokenizer.toString());
    ...
    while (tokenizer.ttype != StreamTokenizer.TT_EOL
      ...
      if (tokenizer.ttype == ',' || tokenizer.ttype == '\t'
        ...
        if (tokenizer.sval.equals(m_MissingValue)
          ...
          try {
            logger.info("Value "+tokenizer.sval);
            if (tokenizer.sval.contains("'")) {
                current.addElement(tokenizer.sval);
            } else {
  ...
  public static void main(String[] args) {
    runFileLoader(new CSVLoader2(), args);
  }
}